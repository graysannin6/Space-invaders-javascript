...
function animate()
{
  ...
  if (frames % 500 === 0) {
    ...
    if(keys.a.pressed && player.position.x >= 0 || keys.leftarrow.pressed && player.position.x >= 0)
    {
        player.velocity.x = -7;
        player.rotation = -0.15;
    }else if(keys.d.pressed && canvas.width >= player.position.x + player.width || keys.rightarrow.pressed && canvas.width >= player.position.x + player.width)
    {
        player.velocity.x = 7;
        player.rotation = 0.15;
    }
...
document.querySelector('.Restartbutton').addEventListener('click',()=>{1
  audio.select.play();
